...
def get_knowledge_quotients(model, datagen, val_acc_model, metric=keras.metrics.categorical_accuracy):
...
def get_knowledge_quotient(model, datagen, val_acc_model, locations):
    ...
    model_reduced = modify_model(model, range(locations[0], locations[1]+1))
