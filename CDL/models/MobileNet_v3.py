...
from CDL.utils.keras_utils import add_regularization
...
def create_mobilenet_v3(input_shape=(32,32,3), num_classes=10, is_pretrained=False, mobilenet_shape=(224,224,3), is_small=True, num_change_strides=0):
    ...
    if is_pretrained:   
        ...
        if is_small:
            mobilenet = MobileNetV3Small(weights='imagenet', include_top=True, classes=num_classes, input_shape=mobilenet_shape, backend=keras.backend, layers=keras.layers, models=keras.models, utils=keras.utils)
        else:
    ...
    return add_regularization(Model(inputs=input_net, outputs=x, name='mobilenetv3'))
